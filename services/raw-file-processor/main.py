...
the SOURCE bucket (source_raw_123456).

For every new object under incoming/:
    ...
    4. Process it  (see process_data()).
    5. Upload the result to the DESTINATION bucket (destination_raw_123456).

Main job: when an ai_news_*.json collection lands, render a professional,
CATEGORISED HTML digest (tabs per category, filter to one category on click)
and save it as processed/ai_news_{date}.html.
...
# One-line audience descriptor per category (falls back to "" if unknown).
CATEGORY_AUDIENCE = {
...
# Lazily created so the module imports without credentials (e.g. for tests).
...
def render_news_html(collection: dict, date_str: str) -> str:
    ...
    # Determine the category order (from the collection, else first-seen order).
    ...
    # Group items by category, highest-relevance first within each.
    ...
    for c in grouped:
        ...
        # Keep only the top N most relevant stories per category.
    ...
    # Tabs: "All" + one per non-empty category.
...
# ===========================================================================
# YOUR BUSINESS LOGIC.
# Input : object name + raw bytes.  Output: (bytes to write, output name).
# ===========================================================================
def process_data(object_name: str, raw_bytes: bytes) -> tuple[bytes, str]:
    base = object_name.split("/")[-1]

    # --- News JSON -> professional, categorised HTML digest -----------------
    ...
    # --- CSV demo (kept): append a processed_at column ----------------------
